...
@section('content')
    <div class="container-xxl">
        <x-breadcrumb :breadcrumbs="[['label' => 'văn bản pháp luật', 'url' => '/admin/legal-documents'], ['label' => $title]]" />
            ...
            <div class="row">
                            ...
                            <div class="row gy-3">
                                <div class="col-md-12">
                                    <div class="position-relative">
                                        <x-input label="Tiêu đề bài biết" value="{{ optional($legalDocument)->title }}"
                                            name="title" required />
                                    </div>
                                    <small class="mt-1">
                                        Liên kết tĩnh:
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea name="content" class="ckeditor" id="content">{!! optional($legalDocument)->content !!}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <x-card title="Tối ưu hóa công cụ tìm kiếm">
                        <div class="seo-preview" v-pre="">
                            ...
                            <div class="existed-seo-meta d-none">
                                ...
                                <div>
                                    @php
                                        $date = optional($legalDocument)->created_at ?? now();
                                    @endphp
                        ...
                        <div class="seo-edit-section">
                            ...
                            <div class="mb-3 position-relative">
                                <x-input name="meta_title" value="{{ optional($legalDocument)->meta_title }}"
                                    label="Tiêu đề SEO" />
                            </div>

                            <div class="mb-3 position-relative">
                                <x-textarea name="meta_description" value="{{ optional($legalDocument)->meta_description }}"
                                    label="Mô tả SEO" />
                            </div>
                        </div>
                    </x-card>
                ...
                <div class="col-md-3">
                    ...
                    <x-card title="Trạng thái">
                        <x-select :value="optional($legalDocument)->published ?? 1" name="published" :options="['1' => 'Hoạt động', '0' => 'Ngưng hoạt động']" placeholder="trạng thái" />
                    </x-card>

                    <x-card title="Tài liệu">
                        <input type="file" name="file" class="form-control">

                        @if (optional($legalDocument)->file)
                            <a href="{{ Storage::url($legalDocument->file) }}"
                                class="fst-italic text-decoration-underline mt-1 d-inline-block" target="_blank">Xem tài
                                liệu</a>
                        @endif
                    </x-card>
    ...
    <script>
        $(function() {
            ...
            $('input[name="title"]').on('input', function() {
                const appUrl = "{{ config('app.url') }}/van-ban-phap-luat";
            ...
            submitForm('#myForm', function(res, form, submitAction) {
                const isCreating = @json(!$legalDocument);
                ...
                window.location.href = '/admin/legal-documents';
            });
